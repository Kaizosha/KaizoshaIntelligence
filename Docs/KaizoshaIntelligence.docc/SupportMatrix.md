...
- Anthropic prompt caching is available through Anthropic-specific provider options for both automatic top-level caching and explicit cache breakpoints on supported prompt blocks.
- Anthropic stable web search is available through `AnthropicProviderOptions.serverTools`; dynamic filtering variants that depend on code execution are not yet exposed.
- Anthropic stable code execution is available through `AnthropicProviderOptions.serverTools` on supported Claude 3.5 Haiku, Claude 3.7, and Claude 4 era model families. Uploaded Anthropic files can be remapped into `container_upload` blocks for code execution with `AnthropicFile.asCodeExecutionFileContent()`.
